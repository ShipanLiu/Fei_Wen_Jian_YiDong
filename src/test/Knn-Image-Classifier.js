...
export default function App() {
  ...
  const startup = async () => {
    ...
    setKnnClassifierModel(knnClassifierModel);
  ...
  const saveKnnClassifierModel = async () => {
    ...
    try {
      ...
      await FileSystem.writeAsStringAsync(datasetLocation, stringDataset, {
  ...
  const loadKnnClassifierModel = async () => {
    ...
    try {
      ...
      let stringDataset = await FileSystem.readAsStringAsync(datasetLocation, {
        encoding: FileSystem.EncodingType.UTF8,
      });
  ...
  function base64ImageToTensor(base64) {
    ...
    // 3D 就够了，因为不是teachable machine, 所以不用4d。
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{width: 224, height: 224}}>
        {hasPermission && (
          <Camera style={{flex: 1}} type={type} ref={camera}></Camera>
        )}
        {!hasPermission && (
          <View
